...
int irudiaKargatu(char *fileName, int sg)
{
	int colorkey;
  SDL_Surface* surface;
  SDL_Renderer* gRenderer = getRenderer(sg);//AITOR_EDIT
...
void irudiakMarraztu(int sg)
{
  int i = 0;

  for (i = 0; i < irudiKop; i++)
  {
    irudiaMarraztu(irudiak[i].texture, &irudiak[i].dest, sg);//AITOR_EDIT
  }
}
//AITOR_EDIT
